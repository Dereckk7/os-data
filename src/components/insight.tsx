...
export const INSIGHT_META: Record<InsightType, { label: string; tone: Tone; bar: string }> = {
  opportunity: {
    label: 'Le DATA OS a détecté une opportunité',
    tone: 'gold',
    bar: 'bg-champagne-500/70',
  },
  recommendation: {
    label: 'Le DATA OS recommande une action',
    tone: 'neutral',
    bar: 'bg-cream/40',
  },
...
export function InsightCard({
  ...
  return (
    ...
    >
      <span
        className={cn('absolute top-4 bottom-4 left-0 w-[2px] rounded-r', meta.bar)}
        aria-hidden
      />
      <div className="flex items-start justify-between gap-3 pl-2">
        <span
          className={cn(
            'flex items-center gap-2 text-[9.5px] font-semibold uppercase tracking-[0.14em]',
            toneText[meta.tone]
          )}
        >
          <AnimatedIcon trigger="mount" delay={0.15}>
        ...
        <GlassBadge
          tone={
            insight.status === 'Nouveau'
              ? 'gold'
              : insight.status === 'Traité'
                ? 'success'
                : 'neutral'
          }
          dot={insight.status === 'Nouveau'}
          pulse={insight.status === 'Nouveau'}
        >
          {insight.status}
        </GlassBadge>
      </div>
      ...
      <button
        ...
        tabIndex={onOpen ? 0 : -1}
      >
        <h3 className="pl-2 text-[15px] font-semibold leading-snug tracking-tight">
          {insight.title}
        </h3>
        <p
          className={cn(
            'mt-1.5 pl-2 text-[13px] leading-relaxed text-cream/52',
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-white/[0.06] pt-3 pl-2">
        <span className="num truncate text-[9.5px] uppercase tracking-[0.12em] text-cream/30">
          {insight.agent} · {insight.time}
        </span>
        <GlassButton
          size="sm"
          variant={insight.type === 'opportunity' ? 'gold' : 'ghost'}
