...
export function useToast() {
  return {
    push: (tone: Tone, title: string, desc?: string) =>
      createToast(title, { tone, description: desc }),
  };
}
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 p-3.5">
        <span className={cn('mt-[1px] shrink-0', toneStyles[tone].cls)}>
          {toneStyles[tone].icon}
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-medium leading-snug">{item.title}</p>
          {item.description && (
            <p className="mt-0.5 text-xs leading-relaxed text-cream/50">{item.description}</p>
          )}
          {item.action && (
            <button
              onClick={() => {
...
export function Toaster() {
  ...
  pushExternal = (title, opts = {}) => {
    ...
    setItems((prev) => [
      ...prev.slice(-3),
      { id, title, remaining: opts.duration ?? 4600, ...opts },
    ]);
    return id;
  };
