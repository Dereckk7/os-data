...
/* ————— Surfaces ————— */
export function GlassSurface({ className, sweep, children, ...rest }: HTMLAttributes<HTMLDivElement> & { sweep?: boolean }) {
...
export interface GlassButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: ButtonVariant; size?: ButtonSize; loading?: boolean; iconLeft?: ReactNode; full?: boolean;
}

export function GlassButton({ variant = "ghost", size = "md", loading = false, iconLeft, full, className, children, disabled, ...rest }: GlassButtonProps) {
  return (
    <button
      className={cn(
      ...
      {...rest}
    >
      {loading ? <Loader2 size={15} strokeWidth={1.75} className="animate-spin" aria-hidden /> : iconLeft}
      {children}
    </button>
  );
}
...
/* ————— Panneau section ————— */
export function GlassPanel({ eyebrow, title, action, children, className, bodyClassName }: {
...
/* ————— Champs ————— */
export function GlassInput({ label, icon, right, error, className, id, ...rest }: InputHTMLAttributes<HTMLInputElement> & {
  label?: string; icon?: ReactNode; right?: ReactNode; error?: string;
}) {
...
export function GlassSelect({ label, value, onChange, options, className, id }: {
...
export function GlassBadge({ tone = "neutral", dot, pulse, children, className }: {
  tone?: Tone; dot?: boolean; pulse?: boolean; children: ReactNode; className?: string;
}) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-[3px] text-[10.5px] font-medium tracking-wide whitespace-nowrap", badgeTones[tone], className)}>
      {dot && <span className={cn("h-[5px] w-[5px] rounded-full bg-current", pulse && "pulse-dot")} aria-hidden />}
      {children}
    </span>
  );
}
...
/* ————— Conteneur d'icône ————— */
export function IconContainer({ children, size = "md", className }: { children: ReactNode; size?: "sm" | "md"; className?: string }) {
  return (
    <span
      className={cn(
...
/* ————— Modale ————— */
export function GlassModal({ open, onClose, title, eyebrow, children, footer, wide }: {
