        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              size="lg"
              tooltip={team.name}
              className="data-[state=open]:bg-white/[0.05]"
            >
              ...
              <ChevronsUpDown
                size={14}
                className="ml-auto shrink-0 text-cream/40 group-data-[state=collapsed]/sidebar:hidden"
              />
            </SidebarMenuButton>
          </DropdownMenuTrigger>
          <DropdownMenuContent
    ...
    <SidebarMenu>
      <SidebarMenuItem>
        <DropdownMenu>
          ...
          >
            <DropdownMenuLabel>
              <span className="block truncate normal-case tracking-normal text-[13px] font-semibold text-cream">
                {user.name}
              </span>
              <span className="num block truncate text-[10px] font-normal text-cream/40">
                {user.email}
              </span>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            ...
            <DropdownMenuItem onClick={onHelp}>
              <HelpCircle size={14} strokeWidth={1.6} /> Aide{' '}
            ...
            <DropdownMenuItem
              ...
              className="text-[#e28d85] focus:text-[#e28d85]"
            >
                ...
                <SidebarMenuItem>
                  <CollapsibleTrigger asChild>
                    <SidebarMenuButton tooltip={g.title}>
                      ...
                      <span className="flex-1 truncate group-data-[state=collapsed]/sidebar:hidden">
                        {g.title}
                      </span>
                      <ChevronRight
                        size={14}
              ...
              return (
                <SidebarMenuItem key={a.id}>
                  <SidebarMenuButton tooltip={a.name} onClick={() => navigate(`/agents/${a.id}`)}>
                    <span
                      className="agent-tile grid size-7 shrink-0 place-items-center rounded-[8px] border"
    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={cn(
          ...
          open
            ? 'border-white/[0.14] bg-white/[0.06] text-cream'
            : 'border-white/[0.07] bg-white/[0.03] text-cream/60 hover:border-white/[0.13] hover:text-cream'
        )}
      >
            ...
            <div className="max-h-80 overflow-y-auto">
              {items.map((n) => (
                <div
                  key={n.id}
                  className={cn(
                    'flex gap-2.5 rounded-[11px] px-3 py-2.5 transition-colors hover:bg-white/[0.04]',
          ...
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem className="hidden md:block">
                <Link to="/dashboard" className="text-cream/45 transition-colors hover:text-cream">
                  Maison Ekwata
                </Link>
              </BreadcrumbItem>
        ...
        {MOBILE_NAV.map((n) => (
          <NavLink key={n.to} to={n.to} end={n.end}>
            {({ isActive }) => (
              <span
                className={cn(
                  'relative flex h-full flex-col items-center justify-center gap-1 text-[9.5px] font-medium tracking-wide transition-colors',
              ...
              >
                ...
                <span
                  className={cn(
                    'absolute top-0 h-[2px] w-8 rounded-b bg-cream transition-opacity',
...
function CommandPaletteShell({
  ...
  const items = useMemo<CommandItem[]>(() => {
    ...
    const reqs: CommandItem[] = requestsQ.data
      ...
      mode === 'light'
        ...
        : mode === 'dark'
          ...
          : mode === 'comfort'
            ...
            : 'light';
    const modeIcon =
      mode === 'light' ? Sun : mode === 'dark' ? Moon : mode === 'comfort' ? Feather : Monitor;
    const actions: CommandItem[] = [
      {
        id: 'a-theme',
        group: 'Actions',
        label: `Passer en thème ${nextMode === 'light' ? 'clair' : nextMode === 'dark' ? 'sombre' : nextMode === 'comfort' ? 'confort' : 'système'}`,
        icon: modeIcon,
        keywords: ['thème', 'theme', 'mode'],
          ...
          >
            ...
            <div className="mt-4 space-y-2">
              {[
                ...
                <div
                  key={s.k}
                  className="flex items-center gap-3 rounded-[11px] border border-white/[0.06] bg-white/[0.02] px-3 py-2.5"
                >
            ...
            <div className="mt-4 rounded-[12px] border border-champagne-500/15 bg-champagne-500/[0.05] p-4">
              ...
              <p className="mt-1 text-xs leading-relaxed text-cream/55">
                support@dataos.app · réponse garantie sous 2 h ouvrées.
              </p>
            </div>
            <p className="num mt-4 text-center text-[10px] text-cream/25">
              DATA OS v2.4.1 — build 8f3k2
            </p>
          </motion.div>
        </div>
      )}
