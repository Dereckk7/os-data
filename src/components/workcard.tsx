...
const TONE: Record<WorkTone, { bar: string; text: string; fill: string; ring: string }> = {
  blue: {
    bar: 'bg-[var(--type-blue)]',
    text: 'text-[var(--type-blue)]',
    fill: 'var(--type-blue)',
    ring: 'border-[var(--type-blue)]',
  },
  violet: {
    bar: 'bg-[var(--type-violet)]',
    text: 'text-[var(--type-violet)]',
    fill: 'var(--type-violet)',
    ring: 'border-[var(--type-violet)]',
  },
  orange: {
    bar: 'bg-[var(--type-orange)]',
    text: 'text-[var(--type-orange)]',
    fill: 'var(--type-orange)',
    ring: 'border-[var(--type-orange)]',
  },
  ...
  gold: {
    bar: 'bg-champagne-500/85',
    text: 'text-champagne-300',
    fill: '#c9b27c',
    ring: 'border-champagne-500/60',
  },
  neutral: {
    bar: 'bg-cream/45',
    text: 'text-cream/55',
    fill: 'rgba(245,245,242,0.45)',
...
export function AgentAvatar({
  ...
  return (
    <span
      ...
      className={cn(
        'agent-tile relative grid shrink-0 place-items-center rounded-full border-[1.5px] border-ink-900',
        working && 'agent-breathe'
      )}
    ...
    >
      ...
      {working && (
        <span className="absolute -right-[1px] -bottom-[1px] h-[7px] w-[7px] rounded-full border border-ink-900 bg-jade pulse-dot" />
      )}
    </span>
  );
}
  ...
  return (
    <li className="flex items-center gap-2.5 py-[3px]">
      {step.state === 'done' ? (
        ...
        <span
          className={cn(
            'relative grid h-[13px] w-[13px] shrink-0 place-items-center rounded-full border',
            TONE[tone].ring
          )}
        >
          <span
            className="h-[5px] w-[5px] rounded-full pulse-dot"
            style={{ color: TONE[tone].fill, backgroundColor: 'currentColor' }}
          />
        </span>
      ) : (
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          'grid h-7 w-7 place-items-center rounded-[8px] transition-all duration-150',
          open
            ? 'bg-white/[0.07] text-cream'
            : 'text-cream/35 hover:bg-white/[0.05] hover:text-cream'
        )}
      >
...
export function WorkCard({
  ...
  menu,
  onQuickAdd,
  quickAddLabel = 'Ajouter une étape',
  onOpen,
  openLabel,
  className,
  footerSlot,
  ...
  return (
    ...
    >
      <span
        className={cn('absolute top-4 bottom-4 left-0 w-[2.5px] rounded-r', t.bar)}
        aria-hidden
      />
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p
            className={cn(
              'num truncate text-[9px] font-semibold uppercase tracking-[0.16em]',
              t.text
            )}
          >
            {eyebrow}
      ...
      <div className="mt-3.5">
        <div className="flex items-center justify-between">
          <span className="text-[9.5px] font-semibold uppercase tracking-[0.14em] text-cream/35">
            Progression
          </span>
      ...
      {steps && steps.length > 0 && (
        <ul className="mt-3">
          {steps.map((s) => (
            <StepRow key={s.id} step={s} tone={tone} />
          ))}
        </ul>
      )}

      <div
        className="mt-4 flex items-center justify-between gap-3 border-t border-white/[0.06] pt-3.5"
