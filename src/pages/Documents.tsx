...
const CATEGORIES: ("Tous" | DocumentCategory)[] = ["Tous", "Contrats", "Factures", "Réservations", "Documents clients", "Rapports", "Procédures", "Autres"];
...
export default function Documents() {
  ...
  const filtered = useMemo(() => {
    let out = category === "Tous" ? documentsQ.data : documentsQ.data.filter((d) => d.category === category);
    const q = query.trim().toLowerCase();
    if (q) out = out.filter((d) => d.name.toLowerCase().includes(q) || d.tags.some((t) => t.toLowerCase().includes(q)) || d.source.toLowerCase().includes(q));
    return out;
    ...
    <div className="space-y-5">
      ...
      <Reveal delay={0.06}>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-1.5" role="group" aria-label="Filtrer par catégorie">
            {CATEGORIES.map((c) => (
              <button
      ...
      {documentsQ.loading ? (
        <div className="space-y-2.5">
          {Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-[58px]" />)}
        </div>
      ) : filtered.length === 0 ? (
        <GlassSurface>
          <EmptyState
            ...
            action={<GlassButton variant="gold" size="sm" onClick={() => toast.gold("Import de documents", { description: "Glissez vos fichiers dans la source Documents (simulation)." })}>Connecter la source Documents</GlassButton>}
          />
        </GlassSurface>
      ) : (
        ...
        footer={
          preview && (
            <>
              ...
              <GlassButton variant="primary" iconLeft={<Download size={14} strokeWidth={1.75} />} onClick={() => toast.success("Téléchargement lancé", { description: `${preview.name} · ${preview.size}` })}>
                Télécharger
              </GlassButton>
            </>
          )
