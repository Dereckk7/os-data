...
export default function Validation() {
  ...
  const decide = (
    ...
    toast[tone](label, {
      description: a.title,
      action: { label: 'Annuler', onClick: () => setStatus(a.id, undo) },
    });
  };
      ...
      <Reveal>
        <header>
          ...
          <p className="mt-1.5 max-w-xl text-[13.5px] leading-relaxed text-cream/50">
            Le DATA OS peut travailler seul, mais l'entreprise garde le contrôle des actions
            sensibles.
          </p>
        </header>
      </Reveal>
      ...
      <section aria-label="Actions en attente">
        <Reveal delay={0.05}>
          <div className="mb-3 flex flex-wrap items-baseline gap-2.5">
            ...
            <span className="num text-[22px] font-semibold text-champagne-300">
        ...
        {pending.length === 0 ? (
          <GlassSurface>
            <EmptyState
              ...
              desc="Aucune action sensible en attente. Les agents continuent de travailler en autonomie."
            />
          </GlassSurface>
        ) : filteredPending.length === 0 ? (
          <GlassSurface>
            <EmptyState
              title="Aucune action dans cette catégorie."
              desc="Changez de filtre pour voir les autres actions en attente."
            />
          </GlassSurface>
        ) : (
          <div className="grid gap-4 lg:grid-cols-2">
            {filteredPending.map((a, i) => (
              <Reveal key={a.id} delay={0.04 * i}>
                <motion.article
                  layout
                  className="glass glass-sweep flex h-full flex-col p-5"
                  aria-label={a.title}
                >
                  <div className="flex items-start justify-between gap-3">
                    <span className="num text-[9px] uppercase tracking-[0.14em] text-cream/35">
                      {a.agent} · {a.time}
                    </span>
                    <GlassBadge tone={a.category === 'Actions financières' ? 'warning' : 'neutral'}>
                      {a.category}
                    </GlassBadge>
                  </div>
                  ...
                  <dl className="mt-3.5 space-y-2.5 text-xs">
                    <div>
                      <dt className="text-[9px] font-semibold uppercase tracking-[0.14em] text-cream/35">
                        Pourquoi ?
                      </dt>
                    ...
                    <div>
                      ...
                      <dd className="num mt-0.5 text-[10.5px] leading-relaxed text-cream/45">
                        {a.data}
                      </dd>
                    </div>
                    <div>
                  ...
                  <div
                    className="mt-4 grid grid-cols-3 gap-2 border-t border-white/[0.06] pt-4"
        ...
        <Reveal delay={0.1}>
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <SegmentedControl
              ...
              size="sm"
              options={(['Validées', 'Rejetées', 'Modifiées', 'Automatiques'] as HistoryTab[]).map(
                (h) => ({ value: h, label: h })
              )}
            />
          </div>
        ...
        <GlassSurface className="overflow-hidden p-0">
          {history.length === 0 ? (
            <p className="px-6 py-10 text-center text-[13px] text-cream/40">
              {historyTab === 'Automatiques'
                ? 'Les actions automatiques (sans risque) seront journalisées ici.'
                : `Aucune action ${historyTab.toLowerCase()} pour le moment.`}
            </p>
          ) : (
              ...
              {history.map((a, i) => (
                ...
                >
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="num mt-0.5 block text-[9.5px] uppercase tracking-[0.1em] text-cream/30">
                      {a.agent} · {a.category} · {a.time}
                    </span>
                  </span>
                  <GlassBadge
                    tone={
