...
import { INSIGHT_META, InsightCard } from '../components/insight';
...
export default function Insights() {
  ...
  const filtered = useMemo(
    () =>
      list.filter(
        (i) => (type === 'Tous' || i.type === type) && (status === 'Tous' || i.status === status)
      ),
    [list, type, status]
  );
  ...
  const counts = (t: InsightType | 'Tous') =>
    t === 'Tous' ? list.length : list.filter((i) => i.type === t).length;
      ...
      <Reveal>
        <header>
          ...
          <p className="mt-1.5 max-w-xl text-[13.5px] leading-relaxed text-cream/50">
            Opportunités, anomalies et décisions — détectées en continu par vos agents sur
            ...
            {STATUS_FILTERS.map((s) => (
              <button
                ...
                className={cn(
                  ...
                  status === s
                    ? 'border-white/[0.2] bg-white/[0.08] text-cream'
                    : 'border-white/[0.07] text-cream/45 hover:text-cream/75'
                )}
              >
                {s}
              </button>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filtered.map((i, idx) => (
            <Reveal key={i.id} delay={0.05 * idx}>
              <InsightCard
                insight={i}
                onCta={(ins) => setSelected(ins)}
                onOpen={(ins) => setSelected(ins)}
                className="h-full"
              />
            </Reveal>
          ))}
              ...
              <GlassButton
                ...
                onClick={() => {
                  ...
                  toast('Insight archivé', {
                    ...
                    action: {
                      label: 'Annuler',
                      onClick: () => update(selected.id, { status: prev }),
                    },
                  });
                  setSelected(null);
                }}
              ...
              {selected.status !== 'Traité' && (
                <GlassButton
                  ...
                  onClick={() => {
                    ...
                    toast.success('Insight traité', {
                      description: `${selected.agent} a reçu vos instructions.`,
                    });
                    setSelected(null);
                  }}
                >
        ...
        {selected && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <GlassBadge
                tone={
                  selected.impact === 'Élevé'
                    ? 'danger'
                    : selected.impact === 'Moyen'
                      ? 'warning'
                      : 'neutral'
                }
                dot
              ...
              <span className="num text-[10px] uppercase tracking-[0.12em] text-cream/35">
                {selected.agent} · {selected.time}
              </span>
            </div>
            {selected.status === 'Traité' && (
