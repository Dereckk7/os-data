...
const STATUS_FILTERS: ('Tous' | IntegrationStatus)[] = [
  'Tous',
  'Connecté',
  'Déconnecté',
  'Synchronisation',
...
export default function Integrations() {
  ...
  const configure = async (it: Integration) => {
    ...
    const records =
      it.records > 0
        ? it.records + Math.round(20 + Math.random() * 60)
        : Math.round(200 + Math.random() * 800);
    patch(it.id, { status: 'Connecté', lastSync: "à l'instant", records });
    setBusy(false);
    setConfigTarget(null);
    toast.success(`${it.name} connecté`, {
      description: `${fmtInt(records)} données synchronisées.`,
    });
  };
  ...
  return (
    <div className="space-y-5">
      <Reveal>
        <header>
          ...
          <p className="mt-1.5 text-[13.5px] text-cream/50">
            Reliez vos outils métier — les données circulent dans les deux sens.
          </p>
        </header>
      </Reveal>
      ...
      <Reveal delay={0.06}>
        <div
          className="flex flex-wrap items-center gap-1.5 overflow-x-auto pb-0.5"
          role="group"
      ...
      {integrationsQ.loading ? (
        ...
        <GlassSurface>
          <EmptyState
            title="Aucune intégration dans cet état."
            desc="Changez de filtre pour voir l'ensemble de votre écosystème connecté."
          />
        </GlassSurface>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              <GlassSurface sweep className="group flex h-full flex-col p-5">
                <div className="flex items-start justify-between gap-3">
                  <span className="grid h-10 w-10 place-items-center rounded-[11px] border border-white/[0.09] bg-ink-950/60">
                    <span className="num text-[11px] font-semibold tracking-tight text-cream/75">
                      {it.monogram}
                    </span>
                  </span>
                  <GlassBadge
                    ...
                    dot
                    pulse={it.status === 'Synchronisation'}
                  >
                    {it.status}
                  </GlassBadge>
                </div>
                ...
                <div className="mt-3.5 flex items-center justify-between border-t border-white/[0.06] pt-3">
                  <span className="num text-[10px] text-cream/40">
                    {it.records > 0 ? `${fmtInt(it.records)} données` : 'Aucune donnée'} ·{' '}
                    {it.lastSync}
                  </span>
                </div>
                <div className="mt-3">
                  <GlassButton
                    size="sm"
                    variant={it.status === 'Connecté' ? 'ghost' : 'gold'}
                    full
                    iconLeft={<Settings2 size={13} strokeWidth={1.6} />}
                    onClick={() => setConfigTarget(it)}
                  >
                    {it.status === 'Connecté'
                      ? 'Configurer'
                      : it.status === 'Erreur'
      ...
      <GlassModal
        ...
        onClose={() => {
          if (!busy) setConfigTarget(null);
        }}
        eyebrow={configTarget?.status === 'Connecté' ? 'Configuration' : 'Connexion simulée'}
        title={configTarget ? configTarget.name : ''}
        footer={
          configTarget &&
          (configTarget.status === 'Connecté' ? (
            <>
              <GlassButton variant="ghost" onClick={() => setConfigTarget(null)}>
                Fermer
              </GlassButton>
              <GlassButton
                variant="primary"
                onClick={() => {
                  toast.success('Paramètres enregistrés', {
                    description: `${configTarget.name} — synchronisation toutes les 15 min.`,
                  });
                  setConfigTarget(null);
                }}
              >
          ...
          (configTarget.status === 'Connecté' ? (
            <div className="space-y-3">
              {[
                ...
                {
                  k: 'Données indexées',
                  v: configTarget.records > 0 ? fmtInt(configTarget.records) : '—',
                },
            ...
            <p className="text-[13px] leading-relaxed text-cream/55">
              ...
              <span className="mt-2 block text-[10.5px] text-cream/35">
                Environnement de démonstration — connexion simulée.
              </span>
            </p>
          ))}
      </GlassModal>
