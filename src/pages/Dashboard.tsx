      ...
      <Reveal>
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2.5 text-[24px] font-semibold tracking-tight sm:text-[27px]">
              Bonjour, {firstName}
            </h1>
      ...
      <Reveal delay={0.06}>
        <section className="lcard overflow-hidden">
          <div className="flex items-center justify-between gap-3 border-b border-[var(--card-divider)] px-5 py-4">
            ...
            <Link
              to="/reports"
              className="flex items-center gap-1 text-[12px] font-medium text-[var(--card-title)] transition-colors hover:text-[var(--color-cream)]"
            >
                    ...
                    <div className="relative z-10 flex h-full flex-col p-6">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="ai-tag">
                          <Sparkles size={11} strokeWidth={2} /> IA
                        </span>
                      </div>
                      ...
                      <p className="mt-2 text-[13px] font-semibold text-[#ffffff]/90">
                        {opportunity.title}
                      </p>
                      <p className="mt-1 text-[12.5px] leading-relaxed text-[#ffffff]/55">
                        {opportunity.body}
                      </p>
              ...
              {recommendation && (
                <Reveal delay={0.18}>
                  <article className="lcard flex h-full flex-col p-5">
                    ...
                    <p className="mt-1.5 text-[13px] leading-relaxed text-[var(--card-title)]">
                      {recommendation.body}
                    </p>
                    <div className="mt-auto flex items-center justify-between pt-4">
                      ...
                      <button
                        onClick={() => {
                          toast.neutral('Analyse lancée', {
                            description: "L'Agent Client Intelligence détaille la tendance.",
                          });
                          navigate('/insights');
                        }}
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[13.5px] font-medium tracking-tight">
                          {item.title}
                        </span>
                        <span className="mt-0.5 block truncate text-xs text-[var(--card-title)]">
                          {item.desc}
                        </span>
                      </span>
                      <span className="hidden shrink-0 text-right md:block">
                        <span className="num block text-[13.5px] font-semibold">{item.value}</span>
                        <span className="num mt-0.5 block text-[9px] uppercase tracking-[0.12em] text-[var(--card-title)]">
                          {item.agent}
                        </span>
                      </span>
                      ...
                      <ChevronRight
                        size={15}
                        strokeWidth={1.6}
                        className="shrink-0 text-[var(--card-title)] transition-transform duration-200 group-hover:translate-x-0.5 sm:hidden"
                      />
                    </button>
                  </motion.li>
                ))}
              ...
              <ul className="mt-2">
                {activeAgents.map((a) => (
                  <li key={a.id} className="agent-row">
                    ...
                    >
                      <span
                        className="agent-tile grid h-6 w-6 shrink-0 place-items-center rounded-[7px] border"
