    ...
    return (
      <GlassSurface>
        <ErrorState
          ...
          desc="Cette demande n'existe pas ou a été archivée."
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="num text-[11px] tracking-[0.08em] text-cream/40">
                  {request.ref}
                </span>
              ...
              <h1 className="mt-3 text-[20px] font-semibold tracking-tight sm:text-[22px]">
                {request.title}
              </h1>
            ...
            <div className="shrink-0 text-right">
              ...
              <p className="num mt-1.5 text-[22px] font-semibold tracking-tight">
                {request.amountLabel}
              </p>
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        <Reveal delay={0.1} className="lg:col-span-7">
          <GlassPanel
            eyebrow="Options trouvées"
            title="Sélection préparée par l'Agent Réservation"
            className="h-full"
          >
            {request.options.length === 0 ? (
              <div className="py-6 text-center">
                <TypeIcon
                  ...
                  size={18}
                  strokeWidth={1.5}
                  className="mx-auto text-cream/25"
                />
                <p className="mt-3 text-[13px] font-medium">L'agent analyse les options</p>
                <p className="mt-1 text-xs text-cream/45">
                  La recherche partenaires est en cours. Vous serez notifié.
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? 'border-champagne-500/45 bg-champagne-500/[0.05] shadow-[0_0_30px_rgba(201,178,124,0.06)]'
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="num rounded-[6px] border border-white/[0.09] bg-white/[0.04] px-1.5 py-px text-[9px] font-semibold uppercase tracking-[0.12em] text-cream/55">
                              {opt.label}
                            </span>
                            {opt.recommended && (
                              <GlassBadge tone="gold" dot>
                          ...
                          <p className="mt-2 text-[14.5px] font-semibold tracking-tight">
                            {opt.vehicle}
                          </p>
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {opt.features.map((f) => (
                              <span
                                key={f}
                                className="rounded-full border border-white/[0.07] px-2 py-0.5 text-[10.5px] text-cream/50"
                              >
                                {f}
                              </span>
                            ))}
                        ...
                        <div className="text-right">
                          <p className="num text-[19px] font-semibold tracking-tight">
                            {opt.price}
                          </p>
                          <p className="num mt-0.5 text-[9px] uppercase tracking-[0.1em] text-cream/30">
                            par prestation
                          </p>
                        </div>
                      </div>
                      <div className="mt-3.5 flex flex-wrap items-center gap-4 border-t border-white/[0.06] pt-3.5">
                        <div className="min-w-[160px] flex-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] uppercase tracking-[0.12em] text-cream/35">
                              Fiabilité
                            </span>
                            <span className="num text-[11px] font-semibold text-champagne-300">
                              {opt.reliability}%
                            </span>
                          </div>
                          <div className="mt-1.5 h-[3px] overflow-hidden rounded-full bg-white/[0.07]">
                            <div
                              className="h-full rounded-full bg-champagne-500/80 transition-all duration-700"
                              style={{ width: `${opt.reliability}%` }}
                            />
                          </div>
                        </div>
                        {isSelected ? (
                          <GlassBadge tone="gold" dot className="px-3 py-1.5">
                            <Check size={12} strokeWidth={2} /> Sélectionnée
                          </GlassBadge>
                        ) : (
                          <GlassButton
                            size="sm"
              ...
              {approved ? (
                ...
                <>
                  <p className="text-xs leading-relaxed text-cream/50">
                    Validez la sélection pour lancer la confirmation partenaire et notifier le
                    client.
                  </p>
                  <div className="mt-3.5 space-y-2">
                      ...
                      onClick={() => {
                        ...
                        toast.success('Option validée', {
                          ...
                          action: {
                            ...
                            onClick: () => {
                              setApproved(false);
                              toast.neutral('Validation annulée');
                            },
                          },
                        });
                      }}
                    ...
                    {!selectedOption && (
                      <p className="text-center text-[10.5px] text-cream/35">
                        Sélectionnez d'abord une option ci-contre.
                      </p>
                    )}
                    <GlassButton
                      variant="ghost"
                      full
                      iconLeft={<MessageCircle size={14} strokeWidth={1.6} />}
                      onClick={() =>
                        toast.neutral('Message préparé', {
                          description: 'Brouillon WhatsApp ajouté à la file.',
                        })
                      }
                    >
                      Contacter le client
