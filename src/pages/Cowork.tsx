...
function replyFor(prompt: string): { kind: CoworkKind; text: string } {
  ...
  if (p.includes('retard'))
    return { kind: 'late', text: "J'ai identifié les demandes qui dépassent le délai habituel :" };
  if (p.includes('client') || p.includes('vip'))
    return { kind: 'clients', text: 'Voici vos clients à plus forte valeur :' };
  if (p.includes('rapport'))
    return { kind: 'report', text: 'Le rapport mensuel est prêt. Voici la synthèse :' };
  if (p.includes('analyse') || p.includes('demande'))
    return {
      kind: 'analysis',
      text: "Analyse des 30 derniers jours terminée. Ce qu'il faut retenir :",
    };
  return {
    kind: 'text',
    text: "Je peux analyser vos opérations, préparer un rapport, retrouver une information, comparer des périodes ou demander à un agent d'exécuter une tâche. Essayez une suggestion ci-dessous.",
  };
}
...
function AnalysisBlock() {
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      <Sparkline
        data={[62, 71, 68, 84, 92, 88, 104, 112, 108, 126]}
        width={320}
        height={48}
        className="w-full"
      />
      <p className="text-xs leading-relaxed text-cream/50">
        La croissance est portée par les transferts VIP (+18%). L'Agent Veille recommande de
        rétablir la file prioritaire.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="mt-3 space-y-1.5">
      ...
      <p className="pt-1 text-xs leading-relaxed text-cream/50">
        Cause principale : indisponibilité de 2 chauffeurs partenaires. L'Agent Opérations a lancé
        une réassignation.
      </p>
    </div>
  );
        ...
        >
          <span className="min-w-0">
            <span className="flex items-center gap-2 text-xs font-medium">
              ...
              {c.segment === 'VIP' && (
                <GlassBadge tone="gold" className="px-1.5 py-0 text-[8px]">
                  VIP
                </GlassBadge>
              )}
            </span>
  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="rounded-[11px] border border-white/[0.07] bg-white/[0.02] p-3.5">
        ...
        <p className="num mt-2 text-[9px] uppercase tracking-[0.1em] text-cream/30">
          Sources : CRM · Paiement · Calendar — vérifiées à 09:12
        </p>
      </div>
      <div className="rounded-[12px] border border-saffron/25 bg-saffron/[0.05] p-3.5">
        ...
        <p className="mt-1.5 text-xs leading-relaxed text-cream/55">
          Le DATA OS souhaite envoyer le rapport mensuel à 24 clients. Cette action critique ne sera
          jamais exécutée sans votre accord.
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            onClick={onCancel}
            className="inline-flex h-9 items-center gap-1.5 rounded-[9px] border border-white/[0.09] px-3.5 text-xs font-medium text-cream/70 transition-all hover:bg-white/[0.05] hover:text-cream"
          >
          ...
          <button
            onClick={onValidate}
            className="inline-flex h-9 items-center gap-1.5 rounded-[9px] bg-cream px-3.5 text-xs font-semibold text-ink-950 transition-all hover:bg-white active:scale-[0.98]"
          >
...
function ActionChips({ onChip }: { onChip: (label: string) => void }) {
  return (
    <div className="mt-3 flex flex-wrap gap-1.5 border-t border-white/[0.06] pt-3">
      <p className="num w-full text-[8.5px] uppercase tracking-[0.14em] text-cream/30">
        Actions proposées
      </p>
...
export default function Cowork() {
  ...
  const send = (text: string, fromHistory = false) => {
    ...
    setMessages((m) => [
      ...m,
      { id: `u-${Date.now()}`, role: 'user', kind: 'text', text: clean, at: now() },
    ]);
    setThinking(true);
    ...
    window.setTimeout(() => {
      setMessages((m) => [
        ...m,
        { id: `os-${Date.now()}`, role: 'os', kind: reply.kind, text: reply.text, at: now() },
      ]);
      setThinking(false);
      emitPulse(0.5);
    }, 1100);
  ...
  const onChip = (label: string) => {
    if (label === 'Ajouter aux tâches') {
      toast.gold('Tâche créée', {
        description: '« Relire le rapport mensuel » ajoutée à votre file.',
        action: { label: 'Ouvrir', onClick: () => navigate('/tasks') },
      });
    } else if (label === 'Envoyer pour validation') {
      toast('Envoyé pour validation', {
        description: "L'action apparaît dans le centre de validation.",
        action: { label: 'Examiner', onClick: () => navigate('/validation') },
      });
    } else {
      toast.success('Rapport créé', {
        description: "Rapport mensuel — août, 18 pages, prêt à l'export.",
      });
    }
  };
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-cream/50">
              Travaillez avec votre Data OS comme avec un membre de votre équipe.
            </p>
          </div>
          {started && (
            <button
          ...
          <div className="lcard sticky top-0 h-full overflow-y-auto p-3">
            ...
            {HISTORY.map((h) => (
              <div key={h.group} className="mb-3">
                <p className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-cream/30">
                  {h.group}
                </p>
                {h.items.map((it) => (
                  <button
        ...
        <div className="flex min-h-0 flex-col">
            ...
            {!started ? (
              <div className="flex h-full flex-col items-center justify-center px-4 text-center">
                ...
                <p className="mt-1.5 max-w-sm text-[13px] leading-relaxed text-cream/50">
                  Analysez vos opérations, retrouvez une information, préparez un rapport, demandez
                  une action, explorez vos données.
                </p>
                <div className="mt-6 grid w-full max-w-lg grid-cols-1 gap-2.5 sm:grid-cols-2">
                  {SUGGESTIONS.map((s, i) => (
                    ...
                    >
                      <span className="num text-[8.5px] uppercase tracking-[0.16em] text-champagne-300">
                        {s.label}
                      </span>
              ...
              <div className="mx-auto max-w-2xl space-y-5">
                {messages.map((m) => (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        'max-w-[86%] rounded-[14px] px-4 py-3',
                        m.role === 'user' ? 'bg-cream text-ink-950' : 'lcard'
                      )}
                    >
                      <p
                        className={cn(
                          'text-[13px] leading-relaxed',
                          m.role === 'os' && 'text-cream/80'
                        )}
                      >
                        {m.text}
                      </p>
                      ...
                      {m.kind === 'late' && (
                        <LateBlock onOpen={(id) => navigate(`/requests/${id}`)} />
                      )}
                      {m.kind === 'clients' && (
                        <ClientsBlock onOpen={(id) => navigate(`/clients/${id}`)} />
                      )}
                      {m.kind === 'report' && (
                        <ReportBlock
                          onValidate={validateReport}
                          onCancel={() =>
                            toast.neutral('Action annulée', {
                              description: 'Aucun envoi ne sera effectué.',
                            })
                          }
                          onModify={() =>
                            setInput('Apporte ces modifications au rapport mensuel : ')
                          }
                        />
                      )}
                      ...
                      <p
                        className={cn(
                          'num mt-2 text-[8.5px] uppercase tracking-[0.12em]',
                          m.role === 'user' ? 'text-ink-950/45' : 'text-cream/25'
                        )}
                      >
                ...
                {thinking && (
                  ...
                  >
                    ...
                    <span className="lcard flex items-center gap-1.5 rounded-[14px] px-4 py-3.5">
                      ...
                      <span
                        className="pulse-dot h-[5px] w-[5px] rounded-full bg-cream/50"
          ...
          <div className="safe-bottom pt-3">
            <form
              ...
              className="lcard flex items-center gap-2 rounded-[14px] p-2 pl-4"
            >
              <input
                value={input}
