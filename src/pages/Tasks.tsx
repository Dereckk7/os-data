    ...
    if (next.status === 'Terminé') {
      toast.success('Tâche terminée', {
        ...
        action: {
          label: 'Annuler',
          onClick: () =>
            setOverrides((prev) => ({ ...prev, [t.id]: { ...prev[t.id], status: prevStatus } })),
        },
      });
    }
  };
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 text-[13.5px] text-cream/50">
              Le DATA OS transforme ses analyses en travail concret — priorisé et suivi.
            </p>
          </div>
          <button
            onClick={() =>
              toast.gold('Tâche créée', {
                description: '« Nouvelle tâche » ajoutée à la colonne À faire.',
              })
            }
          ...
          {COLUMNS.map((col) => (
            ...
            >
              ...
              <div className="flex-1 space-y-2.5 rounded-[14px] border border-white/[0.05] bg-white/[0.012] p-2">
                {tasks
                  ...
                  .map((t) => (
                    <motion.article
                      ...
                      className={cn(
                        'glass p-3.5 transition-all duration-200 hover:-translate-y-px',
                        col.status === 'Terminé' && 'opacity-70'
                      )}
                    >
                      <div className="flex items-start gap-2">
                        <span className="mt-0.5 text-cream/25">
                        ...
                        <div className="min-w-0 flex-1">
                          <p
                            className={cn(
                              ...
                              col.status === 'Terminé' && 'line-through decoration-cream/30'
                            )}
                          >
                            {t.title}
                          </p>
                          <div className="mt-2 flex flex-wrap items-center gap-1.5">
                            ...
                            {t.count && (
                              <span className="num rounded-full border border-white/[0.08] px-2 py-px text-[9px] text-cream/45">
                                {t.count}
                              </span>
                            )}
                          </div>
                          <div className="mt-2 flex items-center justify-between gap-2 border-t border-white/[0.06] pt-2">
                            <span className="num truncate text-[8.5px] uppercase tracking-[0.12em] text-cream/30">
                              {t.agent} · {t.due}
                            </span>
                            {t.ref && (
                              <button
                          ...
                          <div className="mt-2 flex gap-1.5">
                            {col.status !== 'À faire' && (
                              <button
                                onClick={() => move(t, -1)}
                                className="h-7 flex-1 rounded-[7px] border border-white/[0.08] text-[10px] font-medium text-cream/55 transition-colors hover:bg-white/[0.05] hover:text-cream"
                              >
                                ← Reculer
                              </button>
                            )}
                ...
                {counts(col.status) === 0 && (
                  <p className="rounded-[10px] border border-dashed border-white/[0.08] p-4 text-center text-[11px] text-cream/25">
                    Aucune tâche
                  </p>
                )}
              </div>
      ...
      <Reveal delay={0.1}>
        <GlassSurface className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
          <p className="text-[12.5px] text-cream/55">
            <span className="num font-semibold text-cream">
              {counts('À faire') + counts('En cours')}
            </span>{' '}
            tâches actives ·{' '}
            <span className="num font-semibold text-cream">{counts('En attente')}</span> en attente
            de décision
          </p>
          <GlassBadge tone="gold" dot>
            Les agents mettent à jour cette file automatiquement
          </GlassBadge>
        </GlassSurface>
      </Reveal>
    </div>
