...
export default function Sources() {
  ...
  useEffect(() => {
    ...
    const t = window.setInterval(
      () => setProgress((p) => Math.min(p + 7 + Math.random() * 9, 96)),
      120
    );
  ...
  useEffect(() => {
    ...
    const finish = window.setTimeout(async () => {
      ...
      patchSource(key, {
        ...
        records: res.records,
        lastActivity: 'Première synchronisation réussie',
        error: undefined,
      });
      setConnectTarget(null);
      toast.success(`${connectTarget.name} connecté`, {
        description: `${fmtInt(res.records)} données seront organisées automatiquement.`,
      });
    }, 2700);
    return () => {
  ...
  const sync = async (s: DataSource) => {
    ...
    patchSource(s.key, {
      status: 'connected',
      lastSync: "à l'instant",
      records: s.records + gained,
      lastActivity: `${gained} nouvelles données indexées`,
      error: undefined,
    });
    setSyncingKey(null);
    setProgress(100);
    toast.success(`${s.name} synchronisé`, {
      description: `${fmtInt(s.records + gained)} données vérifiées à l'instant.`,
    });
  };
    ...
    <div className="space-y-5">
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 text-[13.5px] text-cream/50">
              Vos données, connectées et organisées en continu.
            </p>
          </div>
          <GlassButton
            variant="ghost"
            iconLeft={<RefreshCw size={14} strokeWidth={1.6} />}
            disabled={syncingKey !== null}
            onClick={async () => {
              for (const s of sources) if (s.status === 'connected') await sync(s);
            }}
          >
            Tout synchroniser
          </GlassButton>
          ...
          {sources.map((s, i) => (
            <Reveal key={s.id} delay={0.04 * i}>
              <GlassSurface
                sweep
                className={cn(
                  'group flex h-full flex-col p-5 transition-colors duration-300',
                  s.status === 'connected' && 'border-jade/20',
                  s.status === 'error' && 'border-ember/30'
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <AnimatedIcon
                    trigger="hover"
                    className={cn(
                      'grid h-9 w-9 place-items-center rounded-[10px] border border-white/[0.08] bg-ink-950/55',
                      s.status === 'error' ? 'text-[#e28d85]' : 'text-cream/70'
                    )}
                  >
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/[0.06] pt-3.5">
                  {[
                    ...
                    <div key={st.k} className="min-w-0">
                      <p className="text-[8.5px] uppercase tracking-[0.12em] text-cream/30">
                        {st.k}
                      </p>
                      <p
                        className={cn(
                          'mt-0.5 truncate text-[10.5px] font-medium',
                          st.k === 'Données' ? 'num text-cream/80' : 'text-cream/55'
                        )}
                        title={st.v}
                      >
                        {st.v}
                  ...
                  {s.status === 'error' ? (
                    <div>
                      <div className="rounded-[11px] border border-ember/25 bg-ember/[0.07] p-3">
                        <p className="text-[11.5px] font-medium text-[#e28d85]">
                          Cette source n'a pas pu être synchronisée.
                        </p>
                      ...
                      <div className="mt-2.5 flex gap-2">
                        <GlassButton
                          ...
                          variant="soft"
                          full
                          iconLeft={<RefreshCw size={13} strokeWidth={1.75} />}
                          onClick={() => sync(s)}
                          loading={syncingKey === s.key}
                        >
                          Réessayer
                        </GlassButton>
                        <GlassButton size="sm" onClick={() => setErrorTarget(s)}>
                          Détails
                        </GlassButton>
                      </div>
                    </div>
                    ...
                    <div>
                      <GlassButton
                        ...
                        iconLeft={
                          <RefreshCw
                            size={13}
                            strokeWidth={1.75}
                            className={syncingKey === s.key ? 'animate-spin' : ''}
                          />
                        }
                        onClick={() => sync(s)}
                        disabled={syncingKey !== null}
                      ...
                      {syncingKey === s.key && (
                        <div className="mt-2 h-[3px] overflow-hidden rounded-full bg-white/[0.06]">
                          <div
                            className="h-full rounded-full bg-champagne-500/80 transition-all duration-200"
      ...
      <Reveal delay={0.2}>
        <GlassPanel
          ...
          title="Prête pour vos intégrations"
          bodyClassName="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4"
        >
          {[
            ...
            <div
              key={a.label}
              className="rounded-[12px] border border-white/[0.06] bg-white/[0.018] p-3.5"
            >
      ...
      <GlassModal
        ...
        eyebrow="Connexion simulée"
        title={connectTarget ? `Connecter ${connectTarget.name}` : ''}
      >
        <div className="space-y-2.5">
          {OAUTH_STEPS.map((label, i) => {
            ...
            return (
              <div
                key={label}
                className={cn(
                  'flex items-center gap-3 rounded-[11px] border px-3.5 py-3 transition-all duration-300',
                  done
                    ? 'border-jade/25 bg-jade/[0.05]'
                    : current
                      ? 'border-champagne-500/30 bg-champagne-500/[0.05]'
                      : 'border-white/[0.06] bg-white/[0.015] opacity-50'
                )}
              >
                {done ? (
                  <Check size={14} strokeWidth={2} className="shrink-0 text-jade" />
                ) : current ? (
                  <Loader2
                    size={14}
                    strokeWidth={1.75}
                    className="shrink-0 animate-spin text-champagne-300"
                  />
                ) : (
                  <span className="h-[14px] w-[14px] shrink-0 rounded-full border border-white/15" />
                )}
                <span
                  className={cn('text-xs font-medium', done ? 'text-cream/80' : 'text-cream/55')}
                >
                  {label}
                </span>
              </div>
          ...
          <p className="pt-1 text-[10.5px] leading-relaxed text-cream/35">
            Environnement de démonstration — aucun accès réel au fournisseur. Le flux OAuth
            production est câblé via /lib/services.
          </p>
        </div>
      </GlassModal>
      ...
      <GlassModal
        ...
        footer={
          errorTarget && (
            <>
              <GlassButton variant="ghost" onClick={() => setErrorTarget(null)}>
                Fermer
              </GlassButton>
              <GlassButton
                variant="primary"
      ...
      >
        {errorTarget && (
          <div className="space-y-3">
            <ErrorState
              className="!py-4"
              title="Cette source n'a pas pu être synchronisée."
              desc={errorTarget.error}
            />
