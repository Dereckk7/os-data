...
export default function Clients() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q)
      out = out.filter((c) => c.name.toLowerCase().includes(q) || c.city.toLowerCase().includes(q));
    return out;
                ...
                >
                  ...
                  <span className="mt-3 flex items-end justify-between border-t border-white/[0.06] pt-3">
                    <span>
                      ...
                      <span className="text-[9.5px] uppercase tracking-[0.1em] text-cream/30">
                        valeur client
                      </span>
                    </span>
...
function ClientRow({ client: c, onOpen }: { client: Client; onOpen: () => void }) {
  return (
    ...
    >
          ...
          <span className="h-[3px] w-16 overflow-hidden rounded-full bg-white/[0.07]">
            <span
              className={cn(
                ...
                c.activity > 60 ? 'bg-jade/80' : c.activity > 30 ? 'bg-saffron/80' : 'bg-ember/80'
              )}
      ...
      <td className="px-4 py-3">
        <GlassBadge tone={riskTone[c.risk]}>{c.risk}</GlassBadge>
      </td>
