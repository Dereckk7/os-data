...
const KIND_FILTERS: ('Tout' | ActivityKind)[] = [
  'Tout',
  'agent',
  'utilisateur',
  'systeme',
...
const KIND_DOT: Record<ActivityKind, string> = {
  agent: 'bg-champagne-400',
  utilisateur: 'bg-jade',
  systeme: 'bg-cream/40',
  action: 'bg-[var(--type-blue)]',
  erreur: 'bg-ember',
};
      ...
      <Reveal>
        <header>
          ...
          <p className="mt-1.5 text-[13.5px] text-cream/50">
            Le journal global du DATA OS — agents, utilisateurs, système.
          </p>
        </header>
      </Reveal>
          ...
          <ol className="space-y-0">
            {filtered.map((a, i) => (
              <li key={a.id} className="grid grid-cols-[52px_14px_1fr] gap-x-1">
                <span className="num pt-[3px] text-right text-[10.5px] leading-5 text-cream/40">
                  {a.time}
                </span>
                <span className="relative flex justify-center">
                  <span
                    className={cn('z-10 mt-[7px] h-[7px] w-[7px] rounded-full', KIND_DOT[a.kind])}
                  />
                  {i < filtered.length - 1 && (
                ...
                <span className={cn('block', i < filtered.length - 1 ? 'pb-4' : 'pb-0.5')}>
                  ...
                  {a.desc && (
                    <span className="mt-0.5 block text-xs leading-relaxed text-cream/45">
                      {a.desc}
                    </span>
                  )}
