...
const BOARD_COLUMNS: RequestStatus[] = [
  'En recherche',
  'À valider',
  'En attente client',
  'Confirmée',
      ...
      activity: [
        {
          id: 'a1',
          time: "à l'instant",
          title: 'Demande créée et qualifiée',
          agent: 'Agent Réservation',
          desc: "Recherche d'options lancée automatiquement.",
        },
      ],
    };
    ...
    <div className="space-y-5">
      <Reveal>
        <header className="flex flex-wrap items-end justify-between gap-4">
          ...
          <GlassButton
            variant="primary"
            iconLeft={<Plus size={15} strokeWidth={1.75} />}
            ...
            <SegmentedControl<View>
              ...
              options={[
                { value: 'liste', label: 'Liste', icon: <List size={12.5} strokeWidth={1.75} /> },
                {
                  value: 'board',
                  label: 'Board',
                  icon: <Columns size={12.5} strokeWidth={1.75} />,
                },
                {
                  value: 'calendrier',
                  label: 'Mois',
                  icon: <CalendarDays size={12.5} strokeWidth={1.75} />,
                },
                {
                  value: 'cartes',
                  label: 'Cartes',
                  icon: <Layers size={12.5} strokeWidth={1.75} />,
                },
              ]}
            />
          </div>
      ...
      {requestsQ.loading ? (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/[0.07]">
                    {['Client', 'Demande', 'Statut', 'Priorité', 'Responsable', 'Heure'].map(
                      (h) => (
                        <th key={h} className="card-eyebrow px-4 py-3 first:pl-5 last:pr-5">
                          {h}
                        </th>
                      )
                    )}
                      ...
                      <td className="py-3 pl-5 pr-4">
                        <span className="flex items-center gap-2.5">
                          ...
                          <span className="text-[13px] font-medium">
                            {r.client}
                            {r.vip && (
                              <span className="num ml-1.5 rounded-[5px] border border-champagne-500/25 bg-champagne-500/[0.08] px-1.5 py-px text-[8.5px] font-semibold tracking-[0.1em] text-champagne-300">
                                VIP
                              </span>
                            )}
                          </span>
                      ...
                      <td className="num py-3 pl-4 pr-5 text-[11px] text-cream/45">
                        <span className="flex items-center justify-between gap-2">
                          ...
                          <ChevronRight
                            size={14}
                            strokeWidth={1.6}
                            className="text-cream/20 transition-all duration-200 group-hover:translate-x-0.5 group-hover:text-cream"
                          />
                        </span>
                      </td>
                    </tr>
          ...
          <div className="space-y-2.5 md:hidden">
            {filtered.map((r, i) => (
              <Reveal key={r.id} delay={0.03 * i}>
                <button
                  onClick={() => navigate(`/requests/${r.id}`)}
                  className="glass glass-sweep w-full p-4 text-left"
                >
                  <span className="flex items-center justify-between gap-2">
                  ...
                  <span className="mt-2.5 flex items-start gap-2">
                    <TypeIcon
                      ...
                      size={14}
                      strokeWidth={1.5}
                      className="mt-0.5 shrink-0 text-cream/35"
                    />
                  ...
                  <span className="mt-3 flex items-center justify-between border-t border-white/[0.06] pt-2.5">
                    <PriorityBadge priority={r.priority} />
                    <span className="num text-[10px] text-cream/35">
                      {r.ref} · {r.time}
                    </span>
                  </span>
                </button>
                ...
                return (
                  <div key={col} className="w-[262px] shrink-0">
                    <div className="mb-2.5 flex items-center justify-between px-1">
                      <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-cream/45">
                        {col}
                      </span>
                      <span className="num rounded-[6px] border border-white/[0.08] bg-white/[0.03] px-1.5 py-px text-[9.5px] text-cream/50">
                        {items.length}
                      </span>
                    </div>
                    <div className="space-y-2">
                      {items.map((r) => (
                        <button
                          key={r.id}
                          onClick={() => navigate(`/requests/${r.id}`)}
                          className="glass glass-sweep w-full p-3.5 text-left transition-transform duration-200 hover:-translate-y-0.5"
                        >
                          <span className="flex items-start justify-between gap-2">
                            <span className="text-xs font-medium leading-snug">{r.title}</span>
                            {r.vip && (
                              <span className="num shrink-0 rounded-[5px] border border-champagne-500/25 bg-champagne-500/[0.08] px-1.5 py-px text-[8px] font-semibold tracking-[0.1em] text-champagne-300">
                                VIP
                              </span>
                            )}
                          </span>
                      ...
                      {items.length === 0 && (
                        <div className="rounded-[12px] border border-dashed border-white/[0.08] p-4 text-center text-[11px] text-cream/25">
                          Aucune demande
                        </div>
                      )}
                    </div>
        ...
        <Reveal delay={0.08}>
          <div className="grid gap-4 md:grid-cols-2">
            {filtered.map((r, idx) => {
              const tone: WorkTone =
                r.status === 'En retard' || r.priority === 'Critique'
                  ? 'ember'
                  : r.status === 'À valider'
                    ? 'orange'
                    : 'blue';
                  ...
                  <WorkCard
                    ...
                    steps={[
                      { id: 'q', label: 'Demande qualifiée', state: 'done' },
                      {
                        ...
                        label:
                          r.options.length > 0
                            ? `${r.options.length} options trouvées`
                            : "Recherche d'options",
                        state: r.options.length > 0 ? 'done' : 'active',
                      },
                      {
                        id: 'v',
                      ...
                      },
                      {
                        ...
                        state: r.status === 'Confirmée' ? 'done' : 'todo',
                      },
                    ]}
                    agents={
                      agent
                        ? [
                            {
                              ...
                              name: agent.name,
                              tint: agent.tint,
                              working: r.status === 'En recherche' || r.status === 'En retard',
                            },
                          ]
                        : []
                    }
                    ...
                    menu={[
                      ...
                      },
                      {
                        ...
                        onClick: () =>
                          toast('Note ajoutée', {
                            description: `${r.ref} — note visible par les agents.`,
                          }),
                      },
                    ]}
                    onQuickAdd={() =>
                      toast('Agent impliqué', {
                        description: `${r.agent} a repris la main sur ${r.ref}.`,
                      })
                    }
        ...
        <Reveal delay={0.08}>
          <GlassSurface className="p-5 sm:p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="num text-[10.5px] text-cream/40">
                {filtered.length} demandes planifiées
              </span>
            </div>
            <div className="grid grid-cols-7 gap-1.5">
              {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
                <span
                  key={`${d}${i}`}
                  className="pb-1 text-center text-[9.5px] font-semibold uppercase tracking-[0.14em] text-cream/30"
                >
                  {d}
                </span>
              ))}
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isToday
                        ? 'border-champagne-500/40 bg-champagne-500/[0.06]'
                        : 'border-white/[0.05] bg-white/[0.012] hover:border-white/[0.12]'
                    )}
                  >
                    <span
                      className={cn(
                        'num text-[10.5px]',
                        isToday ? 'font-semibold text-champagne-300' : 'text-cream/50'
                      )}
                    >
                      {d}
                    </span>
                    <span className="mt-1 flex w-full flex-col gap-[3px] px-1">
                      {dayReqs.slice(0, 2).map((r) => (
                        <button
                          ...
                          className={cn(
                            ...
                            r.status === 'En retard'
                              ? 'border-ember/30 bg-ember/[0.08] text-[#e28d85]'
                              : r.vip
                                ? 'border-champagne-500/20 bg-champagne-500/[0.07] text-champagne-300'
                                : 'border-white/[0.08] bg-white/[0.04] text-cream/60'
                          )}
                        >
                          {r.title}
                        </button>
                      ...
                      {dayReqs.length > 2 && (
                        <span className="num hidden text-[7.5px] text-cream/40 sm:block">
                          +{dayReqs.length - 2}
                        </span>
                      )}
                    </span>
      ...
      <GlassModal
        ...
        footer={
          <>
            ...
            <GlassButton variant="primary" onClick={createRequest}>
              Créer la demande
            </GlassButton>
          </>
        }
      >
        <div className="space-y-3.5">
          ...
          <GlassSelect
            ...
            value={form.type}
            onChange={(v) => setForm((f) => ({ ...f, type: v as RequestType }))}
            options={(
              [
                'Transfert',
                'Réservation',
          ...
          <GlassInput
            label="Description"
            placeholder="Ex. Transfert privé Paris → Cannes, 4 personnes"
            value={form.desc}
          ...
          <p className="rounded-[10px] border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 text-[11.5px] leading-relaxed text-cream/45">
            L'Agent Réservation qualifiera la demande et lancera la recherche d'options
            automatiquement.
          </p>
        </div>
      </GlassModal>
