...
const taskTone = (s: string) =>
  (s === 'Terminé' ? 'success' : s === 'En cours' ? 'gold' : 'neutral') as
...
export default function AgentDetail() {
    ...
    return (
      <GlassSurface>
        <ErrorState
          ...
          desc="Cet agent n'existe pas ou a été retiré de l'organisation."
  ...
  const toggleRun = () => {
    if (isRunning) {
      ...
      toast.success(`${agent.name} réactivé`, {
        description: "L'agent reprend ses opérations immédiatement.",
      });
    }
  };
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 max-w-xl text-xs leading-relaxed text-cream/45">
                  {agent.description}
                </p>
              </div>
            </div>
      ...
      <Reveal delay={0.14} key={tab}>
            ...
            <GlassPanel eyebrow="Capacités" title="Ce que fait cet agent" className="lg:col-span-7">
              <ul className="space-y-2.5">
                {agent.capabilities.map((c) => (
                  <li
                    key={c}
                    className="flex items-center gap-2.5 rounded-[11px] border border-white/[0.06] bg-white/[0.015] px-3.5 py-2.5"
                  >
              ...
              <p className="num mt-4 text-[9.5px] uppercase tracking-[0.12em] text-cream/30">
            ...
            <GlassPanel eyebrow="Rythme" title="7 derniers jours" className="lg:col-span-5">
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className="num text-[10px] text-cream/35">
                  moyenne {Math.round(agent.week.reduce((a, b) => a + b, 0) / 7)} actions/jour
                </span>
                <GlassBadge tone={isRunning ? 'success' : 'neutral'} dot pulse={isRunning}>
                  {agent.current}
                </GlassBadge>
              </div>
            </GlassPanel>
        ...
        {tab === 'Activité' && (
          <GlassPanel eyebrow="Journal" title="Activité récente">
            <ActivityFeed events={agent.events} />
          </GlassPanel>
        )}
              ...
              {agent.tasks.map((t) => (
                <li key={t.id} className="flex items-center justify-between gap-3 py-3">
                  <span className="min-w-0">
                    ...
                    <span className="num mt-0.5 block text-[9.5px] uppercase tracking-[0.12em] text-cream/30">
                      Échéance · {t.due}
                    </span>
                  </span>
                  <GlassBadge
                    tone={taskTone(t.status)}
                    dot={t.status === 'En cours'}
                    pulse={t.status === 'En cours'}
                  >
                    {t.status}
                  </GlassBadge>
                </li>
          ...
          <GlassPanel eyebrow="Données" title="Sources utilisées par l'agent">
            <ul className="space-y-2.5">
              {agent.sources.map((s) => (
                <li
                  key={s}
                  className="flex items-center justify-between gap-3 rounded-[11px] border border-white/[0.06] bg-white/[0.015] px-3.5 py-3"
                >
                  <span className="text-[13px] font-medium">{s}</span>
                  <Link
                    to="/sources"
                    className="text-[11px] font-medium text-cream/55 transition-colors hover:text-cream"
                  >
                    Gérer →
                  </Link>
                </li>
                ...
                {[
                  ...
                  <div key={m.k}>
                    ...
                    <div className="mt-1.5 h-[3px] overflow-hidden rounded-full bg-white/[0.06]">
                      <div
                        className="h-full rounded-full bg-champagne-500/70 transition-all duration-1000"
                        style={{ width: `${m.pct}%` }}
                      />
                    </div>
                  </div>
        ...
        {tab === 'Configuration' && (
          <GlassPanel eyebrow="Réglages" title={`Configuration — ${agent.name}`}>
            <div className="divide-y divide-white/[0.05]">
              <div className="flex items-center justify-between gap-4 py-3.5">
                <div>
                  ...
                  <p className="mt-0.5 text-[11.5px] text-cream/45">
                    Un agent désactivé ne traite plus aucune tâche.
                  </p>
                </div>
                <Toggle
                  checked={enabled}
                  onChange={(v) => {
                    ...
                    toast(v ? `${agent.name} activé` : `${agent.name} désactivé`, {
              ...
              <div className="flex items-center justify-between gap-4 py-3.5">
                <div>
                  ...
                  <p className="mt-0.5 text-[11.5px] text-cream/45">
                    Standard exécute, Supervisée demande avant d'agir.
                  </p>
                </div>
                <div className="w-44">
                  <GlassSelect
              ...
              <div className="flex items-center justify-between gap-4 py-3.5">
                <div>
                  ...
                  <p className="mt-0.5 text-[11.5px] text-cream/45">
                    Les actions sensibles passent par le centre de validation.
                  </p>
                </div>
                <Toggle
                  checked={humanValidation}
                  onChange={setHumanValidation}
            ...
            <p
              className={cn(
                'num mt-4 text-[9.5px] uppercase tracking-[0.12em]',
                enabled ? 'text-cream/30' : 'text-saffron'
              )}
            >
              {enabled
                ? 'Configuration appliquée à la prochaine tâche'
                : 'Agent en pause — les tâches en cours seront terminées'}
            </p>
          </GlassPanel>
        )}
      </Reveal>
