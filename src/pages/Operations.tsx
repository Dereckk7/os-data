...
export default function Operations() {
  ...
  const agentNames = useMemo(
    () => [
      'Tous',
      ...Array.from(new Set(operationsQ.data.filter((o) => o.agent).map((o) => o.agent as string))),
    ],
    [operationsQ.data]
  );
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 text-[13.5px] text-cream/50">
              Chaque action de chaque agent, tracée et horodatée.
            </p>
          </div>
          <GlassBadge tone="success" dot pulse className="mb-1">
            En direct
          </GlassBadge>
        </header>
      </Reveal>
      ...
      <Reveal delay={0.07}>
        <section aria-label="Opérations en cours">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="num text-[9.5px] uppercase tracking-[0.14em] text-cream/30">
              {mockOpsCards.length} chantiers · mise à jour en direct
            </span>
          </div>
            ...
            {mockOpsCards.map((c, i) => {
              const cardAgents = c.agentIds
                ...
                .map((a) => ({
                  ...
                  name: a!.name,
                  tint: a!.tint,
                  working: c.workingIds.includes(a!.id),
                }));
              return (
                    ...
                    menu={[
                      {
                        ...
                        onClick: () =>
                          toast.neutral("Détail de l'opération", { description: c.title }),
                      },
                      {
                        label: 'Notifier le client',
                        onClick: () =>
                          toast.neutral('Notification préparée', {
                            description: 'Brouillon ajouté à la file.',
                          }),
                      },
                    ]}
                    onQuickAdd={() =>
                      toast('Étape ajoutée', {
                        description: `${c.title} — nouvelle étape transmise aux agents.`,
                      })
                    }
                  />
                </Reveal>
                    ...
                    <span className="flex items-center gap-2.5">
                      <span
                        className={cn(
                          ...
                          a.status === 'Opérationnel' ? 'bg-jade pulse-dot' : 'bg-saffron/80'
                        )}
                      />
                      <span className="text-[12.5px] font-medium">{a.name}</span>
                    </span>
                    <span className="num text-[11px] text-cream/45">
