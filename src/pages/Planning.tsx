...
export default function Planning() {
  ...
  const events = useMemo<CalEvent[]>(() => {
    ...
    return [...APPOINTMENTS, ...fromRequests].sort(
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 text-[13.5px] text-cream/50">
              Rendez-vous, demandes, réservations et échéances — au même endroit.
            </p>
          </div>
          <SegmentedControl
            value={view}
            onChange={setView}
            options={(['Jour', 'Semaine', 'Mois'] as View[]).map((v) => ({ value: v, label: v }))}
          />
        </header>
      </Reveal>
      ...
      {requestsQ.loading ? (
        ...
        <Reveal delay={0.08}>
          <GlassSurface className="p-5 sm:p-6">
            ...
            {dayEvents.length === 0 ? (
              <p className="py-10 text-center text-[13px] text-cream/40">
                Aucun événement aujourd'hui — les nouvelles demandes apparaîtront ici.
              </p>
            ) : (
              <ul className="space-y-2">
                {dayEvents.map((e) => (
                  <li key={e.id}>
                    ...
                    >
                      <span className="num text-[12px] font-semibold text-champagne-300">
                        {e.time}
                      </span>
              ...
              {weekDays.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={cn(
                      'flex min-h-[280px] flex-col rounded-[12px] border p-2.5',
                      isToday
                        ? 'border-champagne-500/40 bg-champagne-500/[0.05]'
                        : 'border-white/[0.06] bg-white/[0.012]'
                    )}
                  >
                    <p
                      className={cn(
                        'num mb-2 text-center text-[10.5px] font-semibold uppercase tracking-[0.1em]',
                        isToday ? 'text-champagne-300' : 'text-cream/45'
                      )}
                    >
                      {label} {d}
                    </p>
                    <div className="space-y-1.5">
                      {dayEvts.map((e) => (
                        <button
                          ...
                          onClick={() => e.kind === 'demande' && navigate(`/requests/${e.id}`)}
                          className={cn(
                            'w-full rounded-[8px] border px-2 py-1.5 text-left transition-all duration-150 hover:border-white/[0.2]',
                            e.kind === 'rdv'
                              ? 'border-white/[0.08] bg-white/[0.03]'
                              : 'border-champagne-500/20 bg-champagne-500/[0.06]'
                          )}
                        >
                          <span className="num block text-[8.5px] text-cream/40">{e.time}</span>
                          <span className="mt-0.5 block truncate text-[10.5px] font-medium leading-tight">
                            {e.title}
                          </span>
                        </button>
                      ))}
                          ...
                          {dayEvts.slice(0, 3).map((e) => (
                            <span
                              ...
                              className={cn(
                                ...
                                e.kind === 'rdv' ? 'bg-cream/35' : 'bg-champagne-400'
                              )}
                            />
                          ))}
                        </span>
