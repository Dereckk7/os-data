...
export default function Onboarding() {
  ...
  const finish = (skipped: boolean) => {
    setOnboarded(true);
    if (skipped) {
      toast.neutral('Configuration reportée', {
        description: 'Vous pourrez connecter vos sources à tout moment.',
      });
    } else {
      toast.gold('DATA OS synchronise vos données', {
        description: 'Première organisation automatique en cours.',
      });
    }
    ...
    <div className="relative z-10 mx-auto min-h-screen w-full max-w-3xl px-5 py-10 sm:px-8 sm:py-14">
      <Reveal>
        ...
        <p className="mt-3 max-w-lg text-[14px] leading-relaxed text-cream/55">
          Les données seront organisées automatiquement dans DATA OS. Aucune configuration technique
          requise — nos agents s'occupent du reste.
        </p>
      </Reveal>
          ...
          return (
            <Reveal key={s.key} delay={0.05 + i * 0.05}>
              <GlassSurface
                sweep
                className={cn(
                  'group flex h-full flex-col gap-3 p-5 transition-colors duration-300',
                  isConnected && 'border-jade/25'
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <AnimatedIcon
                    trigger="hover"
                    className="grid h-9 w-9 place-items-center rounded-[10px] border border-white/[0.08] bg-ink-950/55 text-cream/75"
                  >
                ...
                <div className="mt-auto flex items-center justify-between gap-2 border-t border-white/[0.06] pt-3">
                  {isConnected ? (
                    <>
                      <span className="num text-[10.5px] text-jade">
                        {fmtInt(s.records)} données
                      </span>
                    ...
                    <>
                      <span className="num text-[10px] uppercase tracking-[0.1em] text-cream/30">
                        {isDocs ? 'Import local' : 'OAuth sécurisé'}
                      </span>
                      <GlassButton
                        size="sm"
                        ...
                        iconLeft={
                          isDocs ? (
                            <FolderUp size={13} strokeWidth={1.75} />
                          ) : (
      ...
      >
        <GlassButton variant="ghost" onClick={() => finish(true)}>
          Passer pour l'instant
        </GlassButton>
        <div className="flex items-center justify-between gap-3 sm:justify-end">
          <span className="num text-[10.5px] text-cream/35">
            {connectedCount} source{connectedCount > 1 ? 's' : ''} connectée
            {connectedCount > 1 ? 's' : ''}
          </span>
          <GlassButton
            variant="primary"
