...
function loop() {
  ...
  entries.forEach((e) => {
    const inside =
      smoothX >= e.rect.left - 40 &&
      smoothX <= e.rect.left + e.rect.width + 40 &&
      smoothY >= e.rect.top - 40 &&
      smoothY <= e.rect.top + e.rect.height + 40;
    const wantAlpha = inside ? 1 : 0;
    e.ga += (wantAlpha - e.ga) * 0.12;
    if (e.ga > 0.01) {
      const gx = smoothX - e.rect.left;
      const gy = smoothY - e.rect.top;
      if (Math.abs(gx - e.gx) > 0.5) {
        e.gx = gx;
        e.el.style.setProperty('--gx', `${gx.toFixed(1)}px`);
      }
      if (Math.abs(gy - e.gy) > 0.5) {
        e.gy = gy;
        e.el.style.setProperty('--gy', `${gy.toFixed(1)}px`);
      }
    }
...
function start() {
  ...
  const schedule = () => {
    ...
    scrollTimer = window.setTimeout(refreshRects, 120);
  };
...
export function registerGlass(el: HTMLElement): () => void {
  try {
    if (!window.matchMedia('(pointer: fine)').matches || prefersReducedMotion() || tier === 'low')
      return () => {};
  } catch {
    return () => {};
  }
